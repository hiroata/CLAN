...
const workingNavigationCode = `    <!-- 記事ナビゲーション機能 -->
    <script>
        // 全ブログ記事の配列（アルファベット順）
        const articleList = [
...
function emergencyFix(filePath) {
    try {
        ...
        // 既存のナビゲーションスクリプト全体を削除
        const scriptPattern = /<!--\s*記事ナビゲーション機能\s*-->[\s\S]*?<\/script>/;
        
        if (scriptPattern.test(content)) {
            content = content.replace(scriptPattern, workingNavigationCode);
            fs.writeFileSync(filePath, content, 'utf8');
            console.log(`✅ ナビゲーション修正: ${filename}`);
            return true;
        }
        
        // </body>の前に挿入する場合
        const bodyEndPattern = /<\/body>/;
        if (bodyEndPattern.test(content) && !content.includes('記事ナビゲーション機能')) {
            content = content.replace(bodyEndPattern, workingNavigationCode + '\n</body>');
            fs.writeFileSync(filePath, content, 'utf8');
            console.log(`✅ ナビゲーション追加: ${filename}`);
            return true;
        }
