...
function fixFile(filePath) {
    try {
        ...
        // 重複したgetArticleTitle関数の削除
        const duplicatePattern = /}\s*\/\/ フォールバック: ファイル名から推測[\s\S]*?return filename\.replace\('article-utage-', ''\)\.replace\('\.html', ''\)\.replace\(/-\/g, ' '\);\s*}\s*\/\/ ナビゲーションを初期化/;
        ...
        // 重複したfunction initializeNavigation()の削除
        const duplicateInitPattern = /function initializeNavigation\(\) {[\s\S]*?}[\s\S]*?function initializeNavigation\(\) {/;
        
        if (duplicateInitPattern.test(content)) {
            // 最初のinitializeNavigation関数だけを残す
            content = content.replace(duplicateInitPattern, 'function initializeNavigation() {');
            modified = true;
        }

        // 不完全なgetArticleTitle関数の修正
        const brokenGetArticlePattern = /function getArticleTitle\(filename\)\s*{[\s\S]*?}\s*function getArticleTitle\(filename\)\s*{/;
        
        if (brokenGetArticlePattern.test(content)) {
            // 重複した関数定義を削除
            content = content.replace(brokenGetArticlePattern, 'function getArticleTitle(filename) {');
            modified = true;
        }

        // 正しいgetArticleTitle関数の定義
        const correctGetArticleTitleFunction = `        function getArticleTitle(filename) {
        ...
        // 壊れたgetArticleTitle関数を正しいものに置換
        const brokenFunctionPattern = /function getArticleTitle\(filename\)\s*{[\s\S]*?マッピングから日本語タイトルを返す、存在しない場合はファイル名をそのまま返す[\s\S]*?return titleMapping\[filename\][\s\S]*?}\s*\/\/ フォールバック[\s\S]*?}\s*/;
        
        if (brokenFunctionPattern.test(content)) {
            content = content.replace(brokenFunctionPattern, correctGetArticleTitleFunction + '\n\n        ');
            modified = true;
        }

        if (modified) {
            fs.writeFileSync(filePath, content, 'utf8');
            console.log(`✅ 構文エラーを修正: ${filename}`);
            return true;
        } else {
            console.log(`⚠️  修正不要: ${filename}`);
            return false;
        }
    } catch (error) {
...
function main() {
    ...
    console.log('🚀 JavaScriptの構文エラーを修正開始...');
