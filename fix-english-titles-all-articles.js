...
const newGetArticleTitleFunction = `        function getArticleTitle(filename) {
            // 記事ファイル名と日本語タイトルのマッピング
            const titleMapping = {
                'article-utage-accounting-cloud-bookkeeping.html': 'クラウド会計ソフトでUTAGE活用｜自動化戦略',
                'article-utage-administrative-scrivener-license-permit.html': '行政書士のUTAGE戦略｜許認可チェックリストと申請代行受付',
...
function processFile(filePath) {
    try {
        ...
        if (oldFunctionPattern.test(content)) {
            content = content.replace(oldFunctionPattern, newGetArticleTitleFunction);
            fs.writeFileSync(filePath, content, 'utf8');
            console.log(`✅ 修正完了: ${path.basename(filePath)}`);
            return true;
        } else {
            console.log(`⚠️  getArticleTitle関数が見つからない: ${path.basename(filePath)}`);
            return false;
        }
    } catch (error) {
...
function main() {
    ...
    console.log('🚀 全記事の英語タイトルを日本語に修正開始...');
    ...
    if (successCount === totalFiles) {
        console.log('🎉 すべての記事の英語タイトルを日本語に修正しました！');
    } else {
